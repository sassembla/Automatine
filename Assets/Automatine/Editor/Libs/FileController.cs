    ...
    public class FileController
    {
		...
		*/
        ...
        public static void DeleteFileThenDeleteFolderIfEmpty(string localTargetFilePath)
        {
            ...
            File.Delete(localTargetFilePath + AutomatineSettings.UNITY_METAFILE_EXTENSION);
            var directoryPath = Directory.GetParent(localTargetFilePath).FullName;
            var restFiles = FilePathsInFolder(directoryPath);
            if (!restFiles.Any())
            {
                Directory.Delete(directoryPath, true);
                File.Delete(directoryPath + AutomatineSettings.UNITY_METAFILE_EXTENSION);
            }
        }
		...
		*/
        public static List<string> FilePathsInFolderOnly1Level(string localFolderPath)
        {
            ...
            var filePaths = ConvertSeparater(Directory.GetFiles(localFolderPath)
                    .Where(path => !(Path.GetFileName(path).StartsWith(AutomatineSettings.DOTSTART_HIDDEN_FILE_HEADSTRING)))
                    .ToList());

            if (AutomatineSettings.IGNORE_META) filePaths = filePaths.Where(path => !IsMetaFile(path)).ToList();
        ...
        {
            return source.Select(filePath => filePath.Replace(Path.DirectorySeparatorChar.ToString(), AutomatineSettings.UNITY_FOLDER_SEPARATOR.ToString())).ToList();
        }
		...
		*/
        ...
        {
            if (!head.EndsWith(AutomatineSettings.UNITY_FOLDER_SEPARATOR.ToString())) head = head + AutomatineSettings.UNITY_FOLDER_SEPARATOR;

            if (string.IsNullOrEmpty(tail)) return head;
            if (tail.StartsWith(AutomatineSettings.UNITY_FOLDER_SEPARATOR.ToString())) tail = tail.Substring(1);
        ...
        {
            if (filePath.EndsWith(AutomatineSettings.UNITY_METAFILE_EXTENSION)) return true;
            return false;
        }
    }
}